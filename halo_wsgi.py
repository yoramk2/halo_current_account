...
cloned from serverless_wsgi from Author: Logan Raarup <logan@logan.dk>
"""
import importlib
import json
import os
...
def make_serverless_handler(wsgi_app):
...
def make_handler(wsgi_app):
    """ Factory that builds a Lambda event handler for a given WSGI application
    """
    return lambda event, context: handle_request(
        wsgi_app, event, context
    )
...
serverless_handler = make_serverless_handler(wsgi_app)

# Declare the AWS Lambda event handler
handler = make_handler(wsgi_app)

def handle_request(app, event, context):
    if event.get('halo_current_account_service_task_id', None):
        load_lambda_handler(event, context)
        return {}
    else:
...
def load_lambda_handler(event, context):
    ...
    from halo_current_account_service.api.handler import HaloHandler
    h = HaloHandler()
    h.get_event(event, context)

    total = datetime.datetime.now() - now
    logger.debug('finish load_lambda_handler  time: '+str(total))